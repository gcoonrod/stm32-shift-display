...
A proof-of-concept STM32 based development board designed for experimentation with 74x595 driven 7-segment LED character displays. The sponsored revision of the board is built out as a desk clock, the "STM32 Shift Clock".

> **Building or hacking on this?** [`docs/DEVELOPMENT.md`](./docs/DEVELOPMENT.md) covers prerequisites, build and flash, the USB command set, the pin map, and what is still unimplemented.
...
Due to a generous sponsorship from PCBWay the current hardware revision (rev 2) includes a number of substantial design changes over the first revision:
...
Full schematic for this revision: [STM32ShiftDisplayRev2.pdf](./hardware/assets/STM32ShiftDisplayRev2.pdf)

### Revision Objectives
Issues with the USB connection in the first revision made a new revision necessary in order to support UART communication over the USB connection. While the USB termination bug was the primary driver for spinning a new board, there were also a few other short comings of the original design that became apparent during firmware development. The choice of shift registers to drive the LED displays instead of multiplexing made writing the display driver code simple but gave up the brightness control (from PoV) that comes "for free" with that approach. The 74HC595 has an output enable control signal that _maybe_ could be used to achieve a similar effect, so this revision includes a remapping of some of the signals, including the user LEDs, to pins on the MCU that support hardware PWM. 
...
### A note on revision numbers

This board is called **rev 2** here and in `stm32-shift-display.kicad_sch`, but the
sources disagree and all of them describe this same physical board:

| Source | Says |
| --- | --- |
| Schematic title block, `STM32ShiftDisplayRev2.pdf` | rev 2 |
| Git branch `feature/board-rev2`, commit "Initial rev2 design" | rev 2 |
| `stm32-shift-display.kicad_pcb` title block | rev 1 *(stale)* |
| Photo and model assets — `rev1.jpg`, `board_and_stand_rev1.jpg`, `stm32-shift-display_rev1.step` | rev 1 *(same board)* |

The `.kicad_pcb` title block is corrected only when the board is next opened in
KiCad — see [Hardware files](./docs/DEVELOPMENT.md#hardware-files) for why that is
deliberately deferred.

## Firmware

The firmware keeps time on the MCU's RTC and exposes a small command console over
USB CDC, which [`software/timesync`](./software/timesync) uses to compare and correct
the clock against NTP. The on-device menu and display brightness control are not
finished. [`docs/DEVELOPMENT.md`](./docs/DEVELOPMENT.md) has the command set, the
known defects, and what is left to build.
